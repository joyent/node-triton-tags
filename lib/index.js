...
var PORT_NUM_RE = /[0-9]+/;
var KEY_NAME_RE = /^[a-z][a-z0-9\-]*$/i;
var VAL_RE = PORT_NUM_RE;
var MAX_PORT_NUM = 65535;
...
var validatorFromKey = {
    'triton.cns.services': function validateTritonCnsServices(val) {
        ...
        for (var i = 0; i < svcs.length; i++) {
            ...
            if (dnsLabel.length > 64 || !dnsLabel.match(DNS_NAME_RE)) {
                unsafes.push(dnsLabel);
            }

            if (port && port.length > 0) {
                if (!port.match(PORT_NUM_RE)) {
                    unsafes.push(port);
                } else if (parseInt(port, 10) > MAX_PORT_NUM) {
                    unsafes.push(port);
                }
            }

            if (keyVal && keyVal.length > 0) {
                var args2 = keyVal.split('=');
                var key = args2[0];
                if (!key.match(KEY_NAME_RE)) {
                    unsafes.push(key);
                }

                var value = args2[1];
                if (!value.match(VAL_RE)) {
                    unsafes.push(value);
                }
            }
        }
